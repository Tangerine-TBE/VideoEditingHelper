...
class TWVideoCutContainer @JvmOverloads constructor(
...
) : BaseVideoUi(context, attrs, defStyleAttr), IVideoCutKit, PlayerManager.OnPreviewListener {
    ...
    override fun setOnCutListener(listener: IVideoCutKit.OnCutListener?) {
        if (listener == null) {
    ...
    private fun loadThumbnail() {
        mVideoEditerSDK.let {
            ...
            mScope.launch(Dispatchers.IO) {
                ...
                LogUtils.i("--loadThumbnail----------------${mVideoEditerSDK.txVideoInfo.duration}------$interval------------")
                it.initThumbnailList({ index, timeMs, bitmap ->
                    BackgroundTasks.getInstance().runOnUiThread {
                        ...
                        LogUtils.i("--loadThumbnail-------$timeMs----------$index------------")
                    }
                }, interval)
            }
    ...
    //    LogUtils.i("-----onPreviewProgress--${Thread.currentThread()}----$time-----------------------")
    }

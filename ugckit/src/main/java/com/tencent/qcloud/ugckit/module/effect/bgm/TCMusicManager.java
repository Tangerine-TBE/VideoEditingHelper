...
public class TCMusicManager {
    ...
    public void loadMusicList() {
        ...
        TXCLog.e(TAG, "loadMusicList 1");
        ArrayList<TCMusicInfo> bgmInfoList = new ArrayList<TCMusicInfo>();
        TCMusicInfo info = new TCMusicInfo();
        info.name = "test";
        info.localPath = "/sdcard/shuangshengdao.mp3";
        info.progress = 100;
        info.url = "/sdcard/shuangshengdao.mp3";
        info.status = TCMusicInfo.STATE_DOWNLOADED;
        TXCLog.e(TAG, "loadMusicList 2");
        bgmInfoList.add(info);
        if (mLoadMusicListener != null) {
            TXCLog.e(TAG, "loadMusicList 3");
...
//        });
    ...
    public void downloadMusicInfo(final String bgmName, final int position, String url) {
        LoadMusicListener loadMusicListener = mLoadMusicListener;
        if (loadMusicListener != null) {
            loadMusicListener.onBgmDownloadSuccess(position, url);
        }
